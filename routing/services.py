...
class BingGeocodeService(GeocodeService):
    ...
    def get_geocode(address: Address, payload=None, headers=None):
        ...
        if headers is None:
            headers = {}
        response = BingGeocodeService.__request_geocode(
            location=address, payload=payload, headers=headers
        )
    ...
    def __request_geocode(location: Address, payload=None, headers=None):
        ...
        url = "{BASE_URL}?query={QUERY_STRING}&key={API_KEY}".format(
            BASE_URL=BingGeocodeService.__DEFAULT_URL,
    ...
    def __get_coordinates(response: dict, address: Address):
        if "resourceSets" in response.keys():
            ...
            if resource_sets and "resources" in resource_sets:
                resources = (
                    resource_sets["resources"][0]
                    if len(resource_sets["resources"]) > 0
                    else None
                )
...
class BingMatrixService(MatrixService):
    ...
    @staticmethod
    def build_matrices(
        start: Address, end: list, travel_mode: str = "driving", chunk_size: int = 25
    ) -> bool:
        ...
        url = "{BASE_URL}?key={API_KEY}&distanceUnit={DISTANCE_UNIT}".format(
            BASE_URL=BingMatrixService.__DEFAULT_URL,
        ...
        for index in range(0, len(end), chunk_size):
                    ...
                    if address.latitude is None or address.longitude is None:
                        ...
                        try:
                            (
                                address.latitude,
                                address.longitude,
                            ) = BingGeocodeService.get_geocode(address)
                            address.save()
                        except GeocodeError as err:
                            raise err
                    if (
                        address != start
                        and start.neighbor.relationship(address) is None
                    ):
                        destinations.append(
                            {
            ...
            if len(destinations) > 0:
                ...
                headers = {"Content-Length": "450", "Content-Type": "application/json"}
                ...
                try:
                    origins, destinations, results = BingMatrixService.__get_matrices(
                        response=response.json()
                    )
                    logging.info(f"Retrieve the following matrix {results}")
                    BingMatrixService.__insert_matrices(
                        origins=origins, destinations=destinations, results=results
                    )
                except MatrixServiceError:
                    raise MatrixServiceError(
                        "API Error - Could not build matrices from HTTP request"
                    )
        return True
        ...
        if origins and destinations and results:
            for result in results:
                if "destinationIndex" not in result:
                    raise MatrixServiceError(
                        "API Error - BING Matrix - Invalid key 'destinationIndex'"
                    )
                else:
                ...
                if "originIndex" not in result:
                    raise MatrixServiceError(
                        "API Error - BING Matrix - Invalid key 'originIndex'"
                    )
                else:
