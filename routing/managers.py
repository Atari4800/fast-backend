...
class LocationManager:
                ...
                if address and (address.latitude is None or address.longitude is None):
                    ...
                    try:
                        (
                            address.latitude,
                            address.longitude,
                        ) = BingGeocodeService.get_geocode(address=address)
                        address = address.save()
                    except GeocodeError as err:
                        raise err

                if not BingMatrixService.build_matrices(
                    start=address, end=list(self.__addresses)
                ):
                    logging.info(
                        f"Failed to build matrix between {location} and {self.__locations}"
                    )
    ...
    def get_distance_savings(self, location1: Location, location2: Location):
        ...
        if location1 is None or location2 is None:
            return 0.0
        if (
            self.__depot in self.__locations
            and location1 in self.__locations
            and location2 in self.__locations
        ):
    ...
    def __get_distance_saved(self, location1: Address, location2: Address):
        ...
        """
        return (
            self.__depot.distance(location1)
            + self.__depot.distance(location2)
    ...
    def __get_duration_saved(self, location1: Address, location2: Address):
        ...
        """
        return (
            self.__depot.duration(location1)
            + self.__depot.duration(location2)
            - location1.duration(location2)
        )


class SavingsManager:
    def __init__(self, db_connection: str, depot: Address, locations: list):
        self.__location_manager = LocationManager(
            db_connection=db_connection, depot=depot
        )
        self.__heap, self.__valid_locations, self.__invalid_locations = self.__heapify(
            locations=locations
        )
            ...
            for i in range(len(locations)):
                for j in range(i + 1, len(locations)):
                    if locations[i].uid != locations[j].uid:
                        ...
                        try:
                            saving = self.__location_manager.get_distance_savings(
                                location1=pair.first, location2=pair.last
                            )
                            ...
                            if (
                                pair.first.address.longitude is None
                                or pair.first.address.latitude is None
                            ):
                                invalid_locations.add(pair.first)
                            else:
                                valid_locations.add(pair.first)
                            if (
                                pair.last.address.longitude is None
                                or pair.last.address.latitude is None
                            ):
...
class RouteManager:
    ...
    def __init__(self, db_connection: str):
        try:
            ...
            raise ValueError(
                f"Expecting url format: bolt://user:password@localhost:7687 but got {db_connection}"
            )
            ...
            if not driver.route.is_empty:
                ...
                for customer in driver.route.itinerary:
                    if not customer.is_center and customer.is_assigned:
                        logging.info(
                            f"Customer {customer.uid} at {customer.address} with demand {customer.demand} "
                            f"is assigned"
                        )
            else:
                logging.info(
                    f"Driver {driver.uid} is not used. Capacity = {driver.capacity}"
                )
    ...
    def objective_function_value(self):
        return (
            self.__objective_function_value
            if self.__objective_function_value != sys.maxsize
            else None
        )
    ...
    def __build_driver_heap(self) -> list:
        ...
        if self.__drivers:
            ...
            driver_dictionary = {
                index: driver.capacity for index, driver in enumerate(self.__drivers)
            }
            driver_dictionary = sorted(
                driver_dictionary.items(), key=lambda x: x[1], reverse=True
            )
    ...
    def __build_routes(self):
        ...
        for index in range(RouteManager.__NUMBER_OF_ITERATIONS):
            ...
            solver_status, drivers, locations = self.__build_route_instance(
                savings_manager=savings_manager,
                locations=locations,
                drivers_heap=drivers,
            )
            (
                local_objective_function_distance,
                local_objective_function_duration,
    ...
    def __build_route_instance(
        self, savings_manager: SavingsManager, locations: list, drivers_heap: list
    ):
        ...
        if len(locations) == 1:
            ...
            try:
                ...
                for driver in drivers_heap[::-1]:
                    ...
                    if driver.route.is_open and driver.add(
                        pair=Pair(locations[0], None)
                    ):
                        break
                ...
                if (
                    locations[0].address.latitude is None
                    or locations[0].address.longitude is None
                ):
            ...
            while state != RouteManager._State.TERMINATED:
                ...
                logging.info(
                    f"==========Using minimum of {numbers_of_drivers} drivers=========="
                )
                ...
                for pair in local_savings_manager:
                    try:
                        ...
                        if (
                            pair.first.address.latitude is None
                            or pair.last.address.longitude is None
                        ):
        ...
        if (
            len(assigned_locations_list) == 0
            and assigned_locations_list == locations_to_insert
        ):
            RouteManager.__Response[
                "solver_status"
            ] = RouteManager._Status.NO_LOCATIONS_TO_ASSIGN.value
            RouteManager.__Response[
                "message"
            ] = RouteManager._Status.NO_LOCATIONS_TO_ASSIGN.name
            ...
            RouteManager.__Response[
                "solver_status"
            ] = RouteManager._Status.ALL_LOCATIONS_ASSIGNED.value
            RouteManager.__Response[
                "message"
            ] = RouteManager._Status.ALL_LOCATIONS_ASSIGNED.name
            RouteManager.__Response["description"] = "All locations were assigned."
        elif len(locations_to_insert) == 0 or len(self.__invalid_locations) == len(
            locations_to_insert
        ):
            RouteManager.__Response[
                "solver_status"
            ] = RouteManager._Status.NO_LOCATIONS_ASSIGNED.value
            RouteManager.__Response[
                "message"
            ] = RouteManager._Status.NO_LOCATIONS_ASSIGNED.name
            RouteManager.__Response["description"] = "No address could be geocoded."
        elif (
            len(self.__invalid_locations) == 0
            and assigned_locations_list != locations_to_insert
        ):
            RouteManager.__Response[
                "solver_status"
            ] = RouteManager._Status.OTHER_ERROR.value
            ...
            RouteManager.__Response[
                "solver_status"
            ] = RouteManager._Status.SOME_LOCATIONS_ASSIGNED.value
            RouteManager.__Response[
                "message"
            ] = RouteManager._Status.SOME_LOCATIONS_ASSIGNED.name
            RouteManager.__Response[
                "description"
            ] = "Some addresses could not be geocoded."

        RouteManager.__Response["others"] = [
            address.external_id for address in self.__invalid_locations
        ]
...
class NodeParser:
    ...
    def create_drivers(drivers: list):
        ...
        if drivers:
            for driver in drivers:
                ...
                driver_node = NodeParser.set_availability(
                    driver_node, driver["availability"]
                )
    ...
    def create_departure(departure: str):
        ...
        if departure:
            ...
            if node_departure in Depot.nodes.all():
                node_set = Depot.nodes.filter(
                    external_id=node_departure.external_id,
    ...
    def parse_driver(driver: dict):
        if (
            driver.get("employee_status")
            and driver["employee_status"].lower() == "employee"
        ):
        ...
        if driver.get("duration"):
            driver_node.end_time = driver_node.start_time + datetime.timedelta(
                hours=int(driver["duration"])
            )
        else:
    ...
    def parse_address(geographical_location: dict):
        return Address(
            external_id=geographical_location["id"],
            address=geographical_location["address"],
            city=geographical_location["city"],
            state=geographical_location["state"],
    ...
    def parse_language(language: dict):
        if language.get("name"):
            if language.get("name") in Language.options():
                return Language(
                    external_id=language["id"], language=language.get("name")
                )
            else:
                name = " ".join(
                    [part.capitalize() for part in language.get("name").split()]
                )
